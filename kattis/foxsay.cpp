...
/*
    The Problem:
        ...
        Available artifacts:
            ...
            Recording of the sounds of the jungle
            List of known animal sounds in the jungle
    STEP 1:
        To do so we record all of the sounds of the jungle
            HOWTO:  Read a file of all sounds
                    ...
                    What is the eaiest and the hardest way to do this?
    STEP 2:
...
int main() {
    int numOfCases;
    cin >> numOfCases;      //  Get the first token.
    ...
    while (numOfCases--) {
        ...
        while (recStream >> sound) {    //  read each sound from the recording
        ...
        while (getline(cin, line)) {                    //  read each line.
            if (line == "what does the fox say?") {     //  if we find this text
        ...
        for (const string &s : sounds) {
            ...
            if (knownSounds.find(s) == knownSounds.end()) {
                ...
                if (!first) {       //  Why are we doing this check?
